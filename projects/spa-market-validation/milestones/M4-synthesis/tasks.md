# M4 — Synthesis (Week 6)

**Goal:** Compare findings across interviewees and regions.

## Tasks

| ID | Task | Owner | Status | Outcome |
|----|------|-------|--------|---------|
| M4.1 | Fill comparison matrix | | Not started | [cross-interview-comparison.md](../../outcomes/cross-interview-comparison.md) from [comparison-matrix-template.md](../../interviews/templates/comparison-matrix-template.md) |
| M4.2 | Regional breakdown (VN vs FI vs INT) | | Not started | [outcomes/market-fit-by-region.md](../../outcomes/market-fit-by-region.md) |
| M4.3 | Rank pain points | | Not started | [outcomes/pain-point-severity.md](../../outcomes/pain-point-severity.md) |
| M4.4 | Pricing analysis (price-range reactions) | | Not started | [outcomes/pricing-summary.md](../../outcomes/pricing-summary.md) |
| M4.5 | Content preference analysis | | Not started | [outcomes/content-preference-summary.md](../../outcomes/content-preference-summary.md) |
| M4.6 | Clip 3–5 highlight reels (30–60s) | | Not started | [recordings/README.md#highlight-reels-m46](../../interviews/recordings/README.md#highlight-reels-m46) |

## Synthesis workflow

1. Export all post-interview forms into comparison matrix
2. Tag standout quotes by theme (pain, workflow, demo reaction, pricing, AI concern)
3. Calculate hypothesis pass/fail per [hypotheses.md](../../specs/hypotheses.md)
4. Tally per-band reactions and best-fit picks per region (spreadsheet or manual band overlap)
5. Draft regional summaries; identify beachhead candidate
6. Select highlight clips: strongest pain quotes, positive demo reactions, pricing insights

## Exit criteria

All five outcome documents complete; clear per-region patterns documented.
